...
class Paraphraser:
    def __init__(
        ...
        # device selection for CPU or CUDA if available
    ...
    def generate(
        ...
        if params.get("do_sample", False):
            # Force pure sampling; avoids num_return_sequences <= num_beams constraint
            params["num_beams"] = 1
        ...
        if not params.get("do_sample", False):
            ...
            # Drop sampling-only vars to avoid warnings
